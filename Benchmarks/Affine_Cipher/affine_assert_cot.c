...
int modular_multiplicative_inverse(unsigned int a, unsigned int m)
{
    ...
    // Invariant: Resulting x[1] is the inverse or 0 if not coprime
    return x[1];
}
...
affine_key_t inverse_key(affine_key_t key)
{
    ...
    // Invariant: inverse.a is in [0, ALPHABET_SIZE - 1] after adjustment
    inverse.a += ALPHABET_SIZE;
    inverse.a %= ALPHABET_SIZE;
    assert(inverse.a >= 0 && inverse.a < ALPHABET_SIZE);

    inverse.b = -(key.b % ALPHABET_SIZE) + ALPHABET_SIZE;
    // Invariant: inverse.b is in [1, ALPHABET_SIZE] (never negative)
    assert(inverse.b >= 0 && inverse.b <= ALPHABET_SIZE);
...
void affine_encrypt(char *s, affine_key_t key)
{
    for (int i = 0; s[i] != '\0'; i++)
    {
        // Invariant: s[i] is a printable ASCII character (ASCII 32–126)
        assert(s[i] >= 32 && s[i] <= 126);

        int c = (int)s[i] - Z95_CONVERSION_CONSTANT;
        // Invariant: c is in [0, ALPHABET_SIZE-1]
        ...
        // Invariant: c is in [0, ALPHABET_SIZE-1] before conversion back
        assert(c >= 0 && c < ALPHABET_SIZE);

        s[i] = (char)(c + Z95_CONVERSION_CONSTANT);

        // Invariant: Resulting s[i] is printable ASCII
        assert(s[i] >= 32 && s[i] <= 126);
    }
}
...
void affine_decrypt(char *s, affine_key_t key)
{
    ...
    // Invariant: inverse.a in [0, ALPHABET_SIZE-1], inverse.b in [0, ALPHABET_SIZE]

    for (int i = 0; s[i] != '\0'; i++)
    {
        // Invariant: s[i] is printable ASCII
        assert(s[i] >= 32 && s[i] <= 126);
